...
void FERSMonitor::DoReceive(eudaq::EventSP ev){
	...
	for(auto &block_n: block_n_list){
		...
		EUDAQ_INFO("Monitor > ---------- start dumping (block "+std::to_string(block_n)+")");
		switch ( dataq ) {
			case DTQ_SPECT:
				...
				EUDAQ_INFO("tstamp_us : "+std::to_string(tstamp_us ));
				EUDAQ_INFO("trigger_id: "+std::to_string(trigger_id));
				EUDAQ_INFO("chmask    : "+std::to_string(chmask    ));
				EUDAQ_INFO("qdmask    : "+std::to_string(qdmask    ));
				break;
			case DTQ_TIMING:
				...
				// dump the scalar ones
				EUDAQ_INFO("nhits : "+std::to_string(nhits ));
				break;
			case DTQ_COUNT:
				...
				EUDAQ_INFO("trigger_id : "+std::to_string(trigger_id ));
				EUDAQ_INFO("chmask : "+std::to_string(chmask ));
				EUDAQ_INFO("t_or_counts : "+std::to_string(t_or_counts ));
				EUDAQ_INFO("q_or_counts : "+std::to_string(q_or_counts ));
				break;
			case DTQ_WAVE:
				EUDAQ_WARN("Trying to decode WAVE event, espected "+std::to_string(sizeof(EventWave))+" bytes");
				EventWave = FERSunpack_waveevent(&data);
				EUDAQ_INFO("unpack ok");
				tstamp_us = EventWave.tstamp_us;
				trigger_id= EventWave.trigger_id;
				ns        = EventWave.ns;
				for (int i=0; i<MAX_WAVEFORM_LENGTH; i++)
				...
				EUDAQ_INFO("trigger_id : "+std::to_string(trigger_id ));
				EUDAQ_INFO("tstamp_us : "+std::to_string(tstamp_us ));
				EUDAQ_INFO("ns : "+std::to_string(ns ));
				break;
			case DTQ_TSPECT:
			...
			case DTQ_TEST:
				...
				EUDAQ_INFO("tstamp_us : "+std::to_string(tstamp_us ));
				EUDAQ_INFO("trigger_id : "+std::to_string(trigger_id ));
				EUDAQ_INFO("nwords : "+std::to_string(nwords ));
				break;
			case DTQ_STAIRCASE:
